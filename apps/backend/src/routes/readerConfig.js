...
router.get('/reader-config', async (_req, res) => {
  try {
    const { rows } = await pool.query('SELECT r_index, reader_id, portal FROM reader_config ORDER BY r_index');
    return res.json(rows.map(r => ({ r_index: r.r_index, reader_id: r.reader_id, portal: r.portal })));
  } catch (e) {
